...
- Call `remember` after the user states a durable preference, confirms a design decision, establishes project context, or gives a reusable workflow rule.
...
- `remember` persists text and returns an ID and creation date.
...
- Never claim that a fact was remembered unless the `remember` tool call succeeded.
