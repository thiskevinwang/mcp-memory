...
export interface AdminMemoryStore extends MemoryStore {
  listMemories(
    userId: string,
    options: { page: number; filter?: string },
  ): Promise<AdminMemoryPage>;
  searchMemories(userId: string, query: string): Promise<AdminMemory[]>;
  updateMemoryText(userId: string, id: string, text: string): Promise<boolean>;
...
export class D1BackedMemoryStore implements AdminMemoryStore {
  private readonly now: () => Date;
  private readonly createId: () => string;
...
export function createCloudflareMemoryStore(
  ...
  return database ? new D1BackedMemoryStore({ database, vectors }) : vectors;
}
